import type { IonicVueRouterOptions } from "./types";
export declare const createRouter: (opts: IonicVueRouterOptions) => import("vue-router").Router;
export declare const createWebHistory: (base?: string) => import("vue-router").RouterHistory;
export declare const createWebHashHistory: (base?: string) => import("vue-router").RouterHistory;
export declare const createMemoryHistory: (base?: string) => import("vue-router").RouterHistory;
